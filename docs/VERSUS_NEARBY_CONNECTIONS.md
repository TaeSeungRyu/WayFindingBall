...
- **아동 앱 안전 고려:** 자유 텍스트 입력은 UGC 리스크가 있으므로, **미리 준비한 닉네임(귀여운 동물 + 숫자) 중 선택 또는 짧은 길이 + 비속어 필터**를 권장. 실명/식별자 입력 유도는 피함. (구체 방식은 6번 결정 항목.)
...
- [ ] **"이름" 기능 안전** — 상대에게 보이는 닉네임은 자유 텍스트 UGC가 될 수 있음. **미리 준비한 닉네임 선택**(권장) 또는 길이 제한+비속어 필터. 실명/식별자 입력 유도 금지.
...
3. `data/AppSettings.versusName` + "이름" 설정 화면(닉네임 선택/짧은 입력).
...
- [ ] **"이름" 입력 방식** — 아동 앱 UGC 안전을 위해 (a)미리 준비된 닉네임 선택 vs (b)짧은 자유 입력+비속어 필터. (권장: a)
